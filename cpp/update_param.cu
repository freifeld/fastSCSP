...
__host__ void update_param(const double* image_gpu_double, const int* seg_gpu, superpixel_params* sp_params, superpixel_GPU_helper* sp_gpu_helper, const int nPixels, const int nSps, const int xdim, const int ydim, const int prior_sigma_s, const int prior_count){
    ...
    clear_fields<<<BlockPerGrid2,ThreadPerBlock>>>(sp_params,sp_gpu_helper,nSps);
    sum_by_label<<<BlockPerGrid1,ThreadPerBlock>>>(image_gpu_double, seg_gpu, sp_params,sp_gpu_helper,nPixels, xdim);
    calculate_mu_and_sigma<<<BlockPerGrid2,ThreadPerBlock>>>(sp_params, sp_gpu_helper, nSps, prior_sigma_s, prior_count);    
}
...
__global__ void calculate_mu_and_sigma(superpixel_params*  sp_params, superpixel_GPU_helper* sp_gpu_helper, const int nsuperpixel, const int prior_sigma_s, const int prior_count) {
	...
	if (count_int>0){
	    ...
	    sp_params[k].sigma_s.x = sp_gpu_helper[k].mu_i_sum.x / count;
		sp_params[k].sigma_s.y = sp_gpu_helper[k].mu_i_sum.y / count;
  		sp_params[k].sigma_s.z = sp_gpu_helper[k].mu_i_sum.z / count;	
	...
	if (count_int > 3){	    
	    ...
	    C00 = sp_gpu_helper[k].sigma_s_sum.x - mu_x * mu_x * count;
	    C01 = sp_gpu_helper[k].sigma_s_sum.y - mu_x * mu_y * count;
	    C11 = sp_gpu_helper[k].sigma_s_sum.z - mu_y * mu_y * count;
	}

  C00 = (prior_sigma_s + C00) / (total_count - 3.0);
  C01 = C01 / (total_count - 3);
  C11 = (prior_sigma_s + C11) / (total_count - 3.0);
