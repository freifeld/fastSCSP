...
__host__ void Rgb2Lab(uchar3* image_gpu, double* image_gpu_double, int nPixels){
...
__global__ void rgb_to_lab(uchar3* image_gpu, double* image_gpu_double, int nPts) {
	...
	image_gpu_double[3*t] = lval;
	image_gpu_double[3*t+1] = aval;
	image_gpu_double[3*t+2] = bval;
}


__host__ void Lab2Rgb(uchar3* image_gpu, double* image_gpu_double, int nPixels){
	int num_block = ceil( double(nPixels) / double(THREADS_PER_BLOCK) ); 
	dim3 ThreadPerBlock(THREADS_PER_BLOCK,1);
	dim3 BlockPerGrid(num_block,1);
...
__global__ void lab_to_rgb( uchar3* image_gpu, double* image_gpu_double, int nPixels) {
	int t = threadIdx.x + blockIdx.x * blockDim.x;  
	if (t>=nPixels) return;

    double L = image_gpu_double[3*t];
	double La = image_gpu_double[3*t+1];
	double Lb = image_gpu_double[3*t+2];
